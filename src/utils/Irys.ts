...
export const checkIfFund = async (
  ...
  let costToUpload: any;
  if (isFolder && files) {
    costToUpload = await webIrys.utils.estimateFolderPrice(
      files.map((f) => f.size)
    );
  } else if (!isFolder && fileSize) {
    costToUpload = await webIrys.getPrice(fileSize);
  }
  ...
  const currentAccountBalance = await getAccountAPTBalance({
